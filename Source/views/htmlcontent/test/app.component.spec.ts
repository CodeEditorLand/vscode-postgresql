 ...
 */
function sendDataSets(
	ds: MockDataService,
	batchStartMessage: WebSocketEvent,
	resultMessage: WebSocketEvent,
	result: WebSocketEvent,
	count: number
): void {
	for (let i = 0; i < count; i++) {
		// Send a batch start
		let batchStartEvent = <WebSocketEvent>(
			JSON.parse(JSON.stringify(batchStartMessage))
		);
		...
		let resultSetMessageEvent = <WebSocketEvent>(
			JSON.parse(JSON.stringify(resultMessage))
		);
...
class MockSlickGrid {
	...
	@Output() cellChanged: EventEmitter<{
		...
		row: number;
		newValue: any;
	}> = new EventEmitter<{ column: string; row: number; newValue: any }>();
	@Output() editingFinished: EventEmitter<any> = new EventEmitter();
	@Output() contextMenu: EventEmitter<{ x: number; y: number }> =
		new EventEmitter<{ x: number; y: number }>();

	@Input() topRowNumber: number;
	@Output() topRowNumberChange: EventEmitter<number> =
...
describe("AppComponent", function (): void {
	...
	describe("Basic Startup", () => {
		...
		it("initialized properly", () => {
			...
			expect(messages.className.indexOf("hidden")).toEqual(
				-1,
				"messages not visible"
			);
			expect(
				messages.getElementsByTagName("tbody").length
			).toBeGreaterThan(0, "no table body in messages");
			expect(
				messages
	...
	describe("full initialization", () => {
		...
		it("should have started showing messages after the batch start message", () => {
			let dataService = <MockDataService>(
				fixture.componentRef.injector.get(DataService)
			);
		...
		it("should have initialized the grids correctly", () => {
			...
			expect(results.getElementsByTagName("slick-grid").length).toEqual(
				1
			);
	...
	describe("Message Behavior", () => {
		...
		it("Correctly Displays Simple Messages", () => {
			...
			expect(messageTimeCell.getElementsByTagName("span").length).toEqual(
				1
			); // Time cell should be populated

			let messageValueCell = messageCells[1];
			expect(messageValueCell.classList.contains("errorMessage")).toEqual(
				false
			); // Message is not an error
			expect(messageValueCell.classList.contains("batchMessage")).toEqual(
				false
			); // Message should not be indented
			expect(messageValueCell.getElementsByTagName("a").length).toEqual(
				0
			); // Message should not have a link
		});
		...
		it("Correctly Displays Messages With Indentation", () => {
			...
			expect(messageTimeCell.getElementsByTagName("span").length).toEqual(
				0
			); // Time cell should not be populated
	...
	describe("test icons", () => {
		...
		it("should send save requests when the icons are clicked", () => {
			...
			csvIcon.click();
			expect(dataService.sendSaveRequest).toHaveBeenCalledWith(
				0,
				0,
				"csv",
				[]
	...
	describe("test events", () => {
		...
		it("correctly handles custom events", (done) => {
			...
			let shortcutService = <MockShortcutService>(
				fixture.componentRef.injector.get(ShortcutService)
			);
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.toggleResultPane")
			);
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetSmall);
			dataService.sendWSEvent(messageResultSet);
		...
		it("event toggle message pane", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.toggleMessagePane")
			);
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetSmall);
			dataService.sendWSEvent(messageResultSet);
		...
		it("event copy selection", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.copySelection")
			);
		...
		it("event copy with headers", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.copyWithHeaders")
			);
			spyOn(dataService, "copyResults");
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetSmall);
			...
			setTimeout(() => {
				fixture.detectChanges();
				expect(dataService.copyResults).toHaveBeenCalledWith(
					[],
					0,
					0,
					true
		...
		it("event maximize grid", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.maximizeGrid")
			);
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetBig);
			dataService.sendWSEvent(messageResultSet);
		...
		it("event save as json", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.saveAsJSON")
			);
			spyOn(dataService, "sendSaveRequest");
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetSmall);
		...
		it("event save as csv", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.saveAsCSV")
			);
			spyOn(dataService, "sendSaveRequest");
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetSmall);
		...
		it("event save as excel", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.saveAsExcel")
			);
			spyOn(dataService, "sendSaveRequest");
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetSmall);
		...
		it("event next grid", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.nextGrid")
			);
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetBig);
			dataService.sendWSEvent(messageResultSet);
		...
		it("event prev grid", (done) => {
			...
			spyOn(shortcutService, "buildEventString").and.returnValue("");
			spyOn(shortcutService, "getEvent").and.returnValue(
				Promise.resolve("event.prevGrid")
			);
			dataService.sendWSEvent(messageBatchStart);
			dataService.sendWSEvent(resultSetBig);
			dataService.sendWSEvent(messageResultSet);
