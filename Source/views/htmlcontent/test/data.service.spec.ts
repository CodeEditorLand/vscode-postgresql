...
function urlMatch(
	...
	return request.url &&
		...
		request.url.match(expectedUrl).length === 1
		? true
		: false;
}
...
describe("data service", () => {
		...
		it("correctly threads through the data", (done) => {
			mockbackend.connections.subscribe((conn: MockConnection) => {
				...
				conn.mockRespond(
					new Response(
						new ResponseOptions({
							body: JSON.stringify(mockGetRows1),
						})
					)
				);
			});
	...
	describe("set selection request", () => {
		it("correctly threads through the data", (done) => {
			mockbackend.connections.subscribe((conn: MockConnection) => {
				let isSelectionRequest = urlMatch(
					conn.request,
					/\/setEditorSelection/,
					RequestMethod.Post
				);
	...
	describe("get config", () => {
		it("returns correct data on first request", (done) => {
			let config = <IResultsConfig>(
				JSON.parse(JSON.stringify(mockConfig1))
			);
			delete config.shortcuts;
			mockbackend.connections.subscribe((conn: MockConnection) => {
				let isConfigRequest = urlMatch(
					conn.request,
					/\/config/,
					RequestMethod.Get
				);
				...
				conn.mockRespond(
					new Response(
						new ResponseOptions({
							body: JSON.stringify(mockConfig1),
						})
					)
				);
			});
