...
describe("shortcut service", () => {
	...
	describe("string code for", () => {
		it("should return correct stringCodes", (done) => {
			...
			testPromises.push(
				shortcutService
					.stringCodeFor("event.maximizeGrid")
					.then((result) => {
						expect(result).toMatch(
							/(Ctrl\+Shift\+Alt\+y)|(⌘\+⇧\+⌥\+y)/g
						);
					})
			);
			testPromises.push(
				shortcutService
					.stringCodeFor("event.selectAll")
					.then((result) => {
			...
			testPromises.push(
				shortcutService
					.stringCodeFor("event.saveAsJSON")
					.then((result) => {
	...
	describe("get event", () => {
		it("should return the correct event", (done) => {
			let testPromises = [];
			testPromises.push(
				shortcutService.getEvent("ctrl+alt+r").then((result) => {
					expect(result).toEqual("event.toggleResultPane");
				})
			);
			testPromises.push(
				shortcutService.getEvent("alt+y").then((result) => {
	...
	describe("build event string", () => {
		it("should build a correct string given valid object", () => {
			expect(
				shortcutService.buildEventString({
					ctrlKey: true,
					altKey: true,
					shiftKey: true,
					which: 74,
