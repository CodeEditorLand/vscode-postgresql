...
suite("Query Runner tests", () => {
	...
	setup(() => {
		testSqlOutputContentProvider = TypeMoq.Mock.ofType(
	...
	test("Notification - Query complete", () => {
		...
		mockEventEmitter.setup((x) =>
			x.emit("complete", TypeMoq.It.isAnyString())
		);
		...
		mockEventEmitter.verify(
			(x) => x.emit("complete", TypeMoq.It.isAnyString()),
			TypeMoq.Times.once()
		);
	...
	suite("Copy Tests", () => {
		...
		const finalStringWithHeader =
			"Col1" + TAB + "Col2" + CLRF + finalStringNoHeader;
