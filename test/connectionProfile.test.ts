...
suite("Connection Profile tests", () => {
	...
	test("CreateProfile should ask questions in correct order", (done) => {
		...
		ConnectionProfile.createProfile(prompter.object).then(
			(profile) => (profileReturned = profile)
		);
	...
	test("Profile is connected to and validated prior to saving", (done) => {
		...
		connectionManagerMock
			.setup((x) => x.connect(TypeMoq.It.isAny(), TypeMoq.It.isAny()))
			.returns(() => Promise.resolve(true));

		let connectionStoreMock = TypeMoq.Mock.ofType(ConnectionStore);
		connectionStoreMock
			.setup((x) => x.saveProfile(TypeMoq.It.isAny()))
		...
		prompter
			...
			.returns((questions) => {
				...
				answers[LocalizedConstants.authTypePrompt] =
		...
		connectionUI
			...
			.then((profile) => {
				...
				connectionManagerMock.verify(
					(x) => x.connect(TypeMoq.It.isAny(), TypeMoq.It.isAny()),
					TypeMoq.Times.once()
				);

				// profile is saved
				connectionStoreMock.verify(
					(x) => x.saveProfile(TypeMoq.It.isAny()),
					TypeMoq.Times.once()
				);
