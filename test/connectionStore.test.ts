...
suite("ConnectionStore tests", () => {
	...
	setup(() => {
		...
		workspaceConfiguration =
			stubs.createWorkspaceConfiguration(configResult);
		vscodeWrapper
	...
	test("formatCredentialId should handle server, DB and username correctly", () => {
		try {
			...
			assert.fail(
				"Expected exception to be thrown when server name missing"
			);
		} catch (e) {
			// Expected
		}
		...
		assert.strictEqual(
			ConnectionStore.formatCredentialId(serverName),
			`Microsoft.SqlTools|itemtype:${profileType}|server:${serverName}`
		);
		assert.strictEqual(
			ConnectionStore.formatCredentialId(serverName, dbName),
			`Microsoft.SqlTools|itemtype:${profileType}|server:${serverName}|db:${dbName}`
		);
		assert.strictEqual(
			ConnectionStore.formatCredentialId(serverName, dbName, userName),
	...
	test("getPickListDetails - details are left empty", () => {
		assert.strictEqual(
			connectionInfo.getPicklistDetails(defaultNamedProfile),
			undefined
		);
	});
	...
	test("getPickListLabel lists server name by default", () => {
		...
		let label = connectionInfo.getPicklistLabel(
			unnamedProfile,
			interfaces.CredentialsQuickPickItemType.Profile
		);
	...
	test("SaveProfile should not save password if SavePassword is false", () => {
		// Given
		globalstate
			.setup((x) => x.get(TypeMoq.It.isAny()))
			.returns((key) => []);

		let credsToSave: interfaces.IConnectionProfile[];
		connectionConfig
			.setup((x) => x.addConnection(TypeMoq.It.isAny()))
		...
		let profile: interfaces.IConnectionProfile = Object.assign(
		...
		return connectionStore
			...
			.then((savedProfile) => {
				...
				assert.ok(
					credsToSave !== undefined && credsToSave.length === 1
				);
	...
	test("SaveProfile should save password using CredentialStore and not in the settings", (done) => {
		...
		credentialStore
			.setup((x) =>
				x.saveCredential(TypeMoq.It.isAny(), TypeMoq.It.isAny())
			)
		...
		let expectedCredFormat: string = ConnectionStore.formatCredentialId(
			defaultNamedProfile.host,
			defaultNamedProfile.dbname,
			defaultNamedProfile.user
		);
		...
		connectionStore
			...
			.then((savedProfile) => {
				...
				assert.strictEqual(
					capturedCreds.credentialId,
					expectedCredFormat
				);
				assert.strictEqual(
					capturedCreds.password,
					defaultNamedProfile.password
				);
				done();
			})
	...
	test("RemoveProfile should remove password from CredentialStore", (done) => {
		...
		connectionConfig
			.setup((x) => x.removeConnection(TypeMoq.It.isAny()))
			.returns((p) => Promise.resolve(p));

		let capturedCreds: any;
		credentialStore
			.setup((x) => x.deleteCredential(TypeMoq.It.isAny()))
		...
		globalstate
			.setup((x) =>
				x.update(TypeMoq.It.isAnyString(), TypeMoq.It.isAny())
			)
		...
		connectionStore
			...
			.then((success) => {
				...
				assert.ok(success);
				credentialStore.verify(
					(x) => x.deleteCredential(TypeMoq.It.isAny()),
					TypeMoq.Times.once()
				);
				...
				assert.strictEqual(
					capturedCreds.credentialId,
					expectedCredFormat,
					"Expect profiles password to have been removed"
				);
				done();
			})
			.catch((err) => done(new Error(err)));
	...
	function testRemoveProfileWithKeepCredential(
		keepCredentialStore: boolean,
		done: Function
	): void {
		// Given have 2 profiles
		let profile = Object.assign(
	...
	test("RemoveProfile finds and removes profile with no profile name", (done) => {
		// Given have 2 profiles
		let unnamedProfile = Object.assign(
			new ConnectionProfile(),
			defaultNamedProfile,
			{
				profileName: undefined,
		...
		let namedProfile = Object.assign(
			new ConnectionProfile(),
			unnamedProfile,
			{
				profileName: "named",
		...
		workspaceConfiguration.update("connections", [
			defaultNamedProfile,
			unnamedProfile,
			namedProfile,
		]);
		...
		connectionStore
			...
			.then((success) => {
				...
				assert.ok(
					updatedCredentials.every((p) => p !== unnamedProfile),
					"expect profile is removed from creds"
				);
				credentialStore.verify(
					(x) => x.deleteCredential(TypeMoq.It.isAny()),
					TypeMoq.Times.once()
	...
	test("RemoveProfile finds and removes profile with a profile name", (done) => {
		...
		connectionStore
			...
			.then((success) => {
				...
				assert.strictEqual(2, updatedCredentials.length);
				assert.ok(
					updatedCredentials.every((p) => p !== namedProfile),
					"expect profile is removed from creds"
				);
				credentialStore.verify(
					(x) => x.deleteCredential(TypeMoq.It.isAny()),
	...
	test("addRecentlyUsed should limit saves to the MaxRecentConnections amount ", (done) => {
		...
		globalstate
			.setup((x) => x.get(TypeMoq.It.isAny()))
			.returns((key) => creds.slice(0, creds.length));
		globalstate
			.setup((x) =>
				x.update(
					TypeMoq.It.isAnyString(),
					TypeMoq.It.isAnyObject(Array)
				)
			)
			.returns(
				(
					id: string,
					credsToSave: interfaces.IConnectionCredentials[]
				) => {
					creds = credsToSave;
					return Promise.resolve();
				}
		...
		for (let i = 0; i < numCreds; i++) {
			let cred = Object.assign({}, defaultNamedProfile, {
				profileName: defaultNamedProfile.profileName + i,
			});
			promise = promise
				.then(() => {
				...
				.then(() => {
					if (i < maxRecent) {
						assert.equal(
							creds.length,
							i + 1,
							"expect all credentials to be saved when limit not reached"
						);
					} else {
						assert.equal(
							creds.length,
							maxRecent,
							`expect only top ${maxRecent} creds to be saved`
						);
					}
					assert.equal(
						creds[0].host,
						cred.host,
						"Expect most recently saved item to be first in list"
					);
		...
		promise.then(
			() => {
				credentialStore.verify(
					...
					TypeMoq.Times.exactly(numCreds)
				);
				let recentConnections =
					connectionStore.getRecentlyUsedConnections();
	...
	test("addRecentlyUsed should add same connection exactly once", (done) => {
		...
		let promise = Promise.resolve();
		let cred = Object.assign({}, defaultNamedProfile, {
			profileName: defaultNamedProfile.profileName + 1,
		});
		promise = promise
			.then(() => {
			...
			.then(() => {
				assert.equal(
					creds.length,
					2,
					"expect 2 unique credentials to have been added"
				);
				assert.equal(
					creds[0].host,
					cred.host,
	...
	test("addRecentlyUsed should save password to credential store", (done) => {
		...
		promise = promise
			...
			.then(() => {
				...
				assert.ok(
					credId.includes(ConnectionStore.CRED_MRU_USER),
					"Expect credential to be marked as an MRU cred"
				);
				assert.ok(utils.isEmpty(creds[0].password));
			})
			.then(() => {
			...
			.then(() => {
				...
				assert.equal(
					creds.length,
					expectedCredCount,
					`expect ${expectedCredCount} unique credentials to have been added`
				);
			})
			.then(() => {
	...
	test("getPickListItems should display Recently Used then Profiles", (done) => {
		...
		for (let i = 0; i < 3; i++) {
			recentlyUsed.push(
				Object.assign({}, defaultNamedProfile, {
					host: defaultNamedProfile.host + i,
				})
			);
		}
		globalstate
			.setup((x) => x.get(Constants.configRecentConnections))
			.returns((key) => recentlyUsed);

		let profiles: interfaces.IConnectionProfile[] = [
			defaultNamedProfile,
			defaultUnnamedProfile,
		];
		connectionConfig
			.setup((x) => x.getConnections(TypeMoq.It.isAny()))
			.returns(() => profiles);
		...
		let items: interfaces.IConnectionCredentialsQuickPickItem[] =
		...
		for (let recentItem of recentlyUsed) {
			...
			assert.equal(
				items[i].quickPickItemType,
				interfaces.CredentialsQuickPickItemType.Mru
			);
			i++;
		}
		...
		assert.equal(
			items[i].quickPickItemType,
			interfaces.CredentialsQuickPickItemType.NewConnection
		);
	...
	test("can clear recent connections list", (done) => {
		...
		for (let i = 0; i < 3; i++) {
			recentlyUsed.push(
				Object.assign({}, defaultNamedProfile, {
					server: defaultNamedProfile.host + i,
				})
			);
		}
		globalstate
			...
			.returns((key) => recentlyUsed);
		globalstate
			.setup((x) => x.update(TypeMoq.It.isAny(), TypeMoq.It.isAny()))
			.returns((key, value) => {
				recentlyUsed = value;
				return Promise.resolve();
			});
	...
	test("addSavedPassword not lookup password if password already set", (done) => {
		...
		let shouldSave = connectionStore.shouldLookupSavedPassword(
			passwordAlreadySetProfile
		);
	...
	test("addSavedPassword not lookup password if emptyPasswordInput is set", (done) => {
		...
		// When I check whether to add saved password
		let shouldSave =
			connectionStore.shouldLookupSavedPassword(emptyPasswordProfile);
