...
suite("SqlOutputProvider Tests", () => {
	...
	test("RunQuery properly sets up two queries to be run", (done) => {
		...
		contentProvider.displayResultPane = function (
			var1: string,
			var2: string
		): void {
			return;
		...
		assert.equal(
			contentProvider.getResultsMap.get("tsqloutput:" + uri).queryRunner
				.isExecutingQuery,
			true
		);
		assert.equal(
			contentProvider.getResultsMap.get("tsqloutput:" + uri2).queryRunner
				.isExecutingQuery,
			true
		);
	...
	test("onUntitledFileSaved should deleted the untitled file and create a new titled file", (done) => {
		...
		// Ensure all side effects occured as intended
		assert.equal(
			contentProvider.getResultsMap.has("tsqloutput:Test_URI"),
			true
		);
		...
		assert.equal(
			contentProvider.getResultsMap.has("tsqloutput:" + uri),
			false
		);
		assert.equal(
			contentProvider.getResultsMap.get("tsqloutput:" + newUri)
				.queryRunner.isExecutingQuery,
			true
		);
		assert.equal(contentProvider.getResultsMap.size, 1);
	...
	test("onDidCloseTextDocument properly mark the uri for deletion", (done) => {
		...
		assert.equal(
			contentProvider.getResultsMap.get("tsqloutput:" + uri)
				.flaggedForDeletion,
			true
		);
	...
	test("isRunningQuery should return the correct state for the query", (done) => {
		...
		assert.equal(
			contentProvider.isRunningQuery("tsqloutput:" + notRunUri),
			false
		);
		assert.equal(contentProvider.getResultsMap.size, 1);
	...
	test("getQueryRunner should return the appropriate query runner", (done) => {
		...
		assert.equal(
			contentProvider.getResultsMap.get("tsqloutput:Test_URI")
				.queryRunner,
			testedRunner
		);
