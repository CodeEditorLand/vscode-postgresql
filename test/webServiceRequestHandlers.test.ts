...
suite("Web Service Request Handler Tests", () => {
	...
	test("RootHandler properly handles request and renders content", (done) => {
		...
		result
			.setup((x) => x.render(TypeMoq.It.isAny(), TypeMoq.It.isAny()))
			.callback(function (path: string, params: any): void {
		...
		result.verify(
			(x) => x.render(TypeMoq.It.isAny(), TypeMoq.It.isAny()),
			TypeMoq.Times.once()
		);
		done();
	});
	...
	test("RowRequestHandler properly handles request and renders content", (done) => {
		...
		queryRunner
			...
			.returns(() => {
				return new Promise<QueryExecuteSubsetResult>(
					(reject, resolve) => {
	...
	test("EditorSelectionRequestHandler properly handles request and renders content", (done) => {
		...
		// Setup a query runner callback and return type
		queryRunner
			.setup((x) => x.setEditorSelection(TypeMoq.It.isAny()))
			.callback(
				(
					selection: any,
					batchId: any,
		...
		// Ensure proper functions were called
		queryRunner.verify(
			(x) => x.setEditorSelection(TypeMoq.It.isAny()),
			TypeMoq.Times.once()
		);
