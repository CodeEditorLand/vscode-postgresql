...
suite("Telemetry Tests", () => {
	test("Correct version of applicationInsights is installed", () => {
		...
		const appInsightsPackage: any = require(
			ext.extensionPath +
				"/node_modules/vscode-extension-telemetry/node_modules/applicationinsights/package.json"
		);
		...
		assert.ok(
			versionOk,
			"Version of applicationInsights must be greater than or equal to 0.15.19. Detected version was " +
				versionString
		);
	});
	...
	test("Path before /out/ is stripped", () => {
		let errorString =
			"/User/myuser/vscode/extensions/Microsoft.vscode-pgsql-0.0.1/out/src/controller/mainController.js:216.45";
	...
	test("Path without /out/ is retained", () => {
		let errorString =
			"/User/should/never/happen/src/controller/mainController.js:216.45";
		let actualErrorString = Telemetry.FilterErrorPath(errorString);
